...
def sim_cal(embeddings,embeddings2):
    def cos_sim(v1, v2):
        return np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))
    
    for i in range(8, 10, 2):
        val1 = embeddings2[i+1] - embeddings2[i]
        j=10
        val2 = val1 + embeddings2[j]
        for k in range(0, 12):
            vec_cal = cos_sim(val2, embeddings2[k])
            print(f"{words[i+1]} - {words[i]} + {words[j]} = {words[k]}? : {vec_cal}")
        '''
        for k in range(0, 10, 2):
            vec_cal = format(cos_sim(val2, embeddings2[k]), '.2f')
            print(f"{words[i+1]} - {words[i]} + {words[j]} = {words2[k]}? : {vec_cal}")
        '''
...
def main():
    sim_cal(embeddings,embeddings2)
    #cos_cal(embeddings)
    #tsne(embeddings)
    #pca(embeddings)
